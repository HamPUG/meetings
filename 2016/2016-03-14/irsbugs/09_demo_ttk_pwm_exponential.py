#!/usr/bin/env python3
#
# Program: demo_ttk_pwm_exponential.py
#
# Objective: Use a GUI as the loop, and control PWM frequency and duty cycle.
#            The frequency scale selects from a list of frequencies which
#            increment / decrement exponentially.
#
...
frequency_pointer = 4  # 4 = 2**0 = 1Hz. 
duty_cycle = 50  # percent
frequency_list = [2**-4, 2**-3, 2**-2, 2**-1, 2**0, 2**1, 2**2, 2**3, 2**4,
                  2**5, 2**6] # 1/16Hz to 64Hz
...
SIGNAL = 11  # For BOARD method this is the P1 connector Pin number. 
             ...
             # E.g. BCM signal 17 is equates to BOARD pin 11.

TITLE_1 = "Pulse Width Modulation - Exponential Scale"
...
class PWM:
    def __init__(self, master, pwm_channel):
        ...
        self.scale_frequency = ttk.Scale(master, from_=0, 
                to=len(frequency_list) -1, 
                command=self.scale_frequency_change, orient=HORIZONTAL, 
                length=200, value = frequency_pointer) # pointer of 4 is 1 HZ
    ...
    def get_frequency(self):
        '''
        The scale returns a floating point value in the range 0 to 
        len(frequency_list)-1. Convert scale value to integer and use as a
        pointer into the frequency list.
        '''
        #print(self.scale_frequency['value'])
        scale_value = self.scale_frequency['value']
        pointer = int(float(scale_value))
        return frequency_list[pointer]
...
def get_memory_size():
    ...
    ps = Popen(["cat", "/proc/meminfo"], stdout=PIPE)
...
'''
Notes:
The slider values are converted to integers and provide a pointer into the
frequency_list. Each change is a doubling or halving of the previous value. 
