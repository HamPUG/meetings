#!/usr/bin/env python3
#
# Program:      prime_number_locater.py
#
# Objective:    NCEA example program
#               Prime number locater.
...
if int(sys.version[0]) < 3:
    print("Python Version Error: Run program using python3 to support "
          "tkinter.\nExiting...")
    sys.exit()
# Import tkinter, ttk, and ohter tkinter modules
try:
...
PROGRAM = "prime_number_locater.py"
VERSION = "1.0"
TITLE_1 = "Prime Number locater. {} {}".format(PROGRAM, VERSION)
...
METHOD_TUPLE = ("Divide every number", "Skip Even numbers",
                "Modulo 6 with 1 & 5", "Minimal Recursion - Sqrt(n)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1].isdigit():
            ...
            print("Prime number locater options: [start integer] and [range]")
            sys.exit(1)
    else:
    ...
    main_gui = (GUI_Prime_Number_Locater(root, start, range_)
