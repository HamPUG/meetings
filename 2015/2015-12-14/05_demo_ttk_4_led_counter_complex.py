#!/usr/bin/env python3
#
# Program: demo_ttk_4_led_counter_complex.py
#
...
# The list is constructed as LSB to MSB for a more complex iteration through
# the list. Uses: bin_string[::-1][i]
#
...
led_list = [11, 12, 13, 15] # Broadcom BCM GPIO signals. LSB --> MSB
#PIN_ADDRESSING_METHOD = "BCM"  # or BOARD
#led_list = [17, 18, 27, 22] # Broadcom BCM GPIO signals. LSB --> MSB

# Create a string that is a list of the pins / signals
SIGNAL = ", ".join(map(str,led_list))

TITLE_1 = "LED Counter - Complex: LSB to MSB"
TITLE_2 = "LED Counter GUI application for Raspberry Pi - Complex"
INFO_0 = "Number:"
    ...
    def button_count_0_to_15(self):
        ...
        while counter < 16:
            ...
            for i in range(len(led_list)):
                gpio.output(led_list[i], not int(bin_string[::-1][i])) 
            sleep(1)
            counter +=1
...
'''
Notes:
If bin_string is 0001 then bin_string[::-1] is 1000. i.e reversed.
